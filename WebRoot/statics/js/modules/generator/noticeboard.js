...
function editArticle() {
	...
	$.ajax({
		url : baseURL + 'noticeboard/update',
		type : 'POST',
		data : da,
		dataType : 'json'
	}).done(function(r) {
...
function getCompetitonNameData(cId) {
	$("#title").val("");
	$("#content").val("");
...
var vm = new Vue({
	...
	methods : {
		...
		edit : function(event) {
			...
			$("#btn_submit").attr("onclick", "return editArticle();");
		
			$.ajax({
					type : "GET",
					async : true,
					url : baseURL + "noticeboard/info/" + id,
					success : function(r) {
					vm.noticeBoard = r.noticeBoard;
				 t= r.noticeBoard;
				articleCid = t.cId;
				getCompetitonNameData(t.cId);
				var ue = UE.getEditor('content');
				ue.setContent(t.content);
